							   ...
							   const int nverts, const int vertsPerPoly)
{
	...
	unsigned short* firstEdge = (unsigned short*)rcAllocate(sizeof(unsigned short)*(nverts + maxEdgeCount), RC_ALLOC_TEMPORARY);
	if (!firstEdge)
		return false;
	unsigned short* nextEdge = firstEdge + nverts;
	int edgeCount = 0;
	
	rcEdge* edges = (rcEdge*)rcAllocate(sizeof(rcEdge)*maxEdgeCount, RC_ALLOC_TEMPORARY);
	if (!edges)
	{
		rcFree(firstEdge);
		return false;
...
static bool canRemoveVertex(rcContext* ctx, rcPolyMesh& mesh, const unsigned short rem)
{
	...
	int nedges = 0;
	rcScopedDelete<int> edges((int*)rcAllocate(sizeof(int)*maxEdges*3, RC_ALLOC_TEMPORARY));
	if (!edges)
	{
...
static bool removeVertex(rcContext* ctx, rcPolyMesh& mesh, const unsigned short rem, const int maxTris)
{
	...
	int nedges = 0;
	rcScopedDelete<int> edges((int*)rcAllocate(sizeof(int)*numRemovedVerts*nvp*4, RC_ALLOC_TEMPORARY));
	if (!edges)
	{
	...
	int nhole = 0;
	rcScopedDelete<int> hole((int*)rcAllocate(sizeof(int)*numRemovedVerts*nvp, RC_ALLOC_TEMPORARY));
	if (!hole)
	{
	...
	int nhreg = 0;
	rcScopedDelete<int> hreg((int*)rcAllocate(sizeof(int)*numRemovedVerts*nvp, RC_ALLOC_TEMPORARY));
	if (!hreg)
	{
	...
	int nharea = 0;
	rcScopedDelete<int> harea((int*)rcAllocate(sizeof(int)*numRemovedVerts*nvp, RC_ALLOC_TEMPORARY));
	if (!harea)
	{
	...
	rcScopedDelete<int> tris((int*)rcAllocate(sizeof(int)*nhole*3, RC_ALLOC_TEMPORARY));
	if (!tris)
	{
	...
	rcScopedDelete<int> tverts((int*)rcAllocate(sizeof(int)*nhole*4, RC_ALLOC_TEMPORARY));
	if (!tverts)
	{
	...
	rcScopedDelete<int> thole((int*)rcAllocate(sizeof(int)*nhole, RC_ALLOC_TEMPORARY));
	if (!thole)
	{
	...
	rcScopedDelete<unsigned short> polys((unsigned short*)rcAllocate(sizeof(unsigned short)*(ntris+1)*nvp, RC_ALLOC_TEMPORARY));
	if (!polys)
	{
		ctx->log(RC_LOG_ERROR, "removeVertex: Out of memory 'polys' (%d).", (ntris+1)*nvp);
		return false;
	}
	rcScopedDelete<unsigned short> pregs((unsigned short*)rcAllocate(sizeof(unsigned short)*ntris, RC_ALLOC_TEMPORARY));
	if (!pregs)
	{
		ctx->log(RC_LOG_ERROR, "removeVertex: Out of memory 'pregs' (%d).", ntris);
		return false;
	}
	rcScopedDelete<unsigned char> pareas((unsigned char*)rcAllocate(sizeof(unsigned char)*ntris, RC_ALLOC_TEMPORARY));
	if (!pareas)
	{
...
///
/// @see rcAllocatePolyMesh, rcContourSet, rcPolyMesh, rcConfig
bool rcBuildPolyMesh(rcContext* ctx, const rcContourSet& cset, const int nvp, rcPolyMesh& mesh)
{
	...
	rcScopedDelete<unsigned char> vflags((unsigned char*)rcAllocate(sizeof(unsigned char)*maxVertices, RC_ALLOC_TEMPORARY));
	if (!vflags)
	{
	...
	mesh.vertices = (unsigned short*)rcAllocate(sizeof(unsigned short)*maxVertices*3, RC_ALLOC_PERMANENT);
	if (!mesh.vertices)
	{
		ctx->log(RC_LOG_ERROR, "rcBuildPolyMesh: Out of memory 'mesh.verts' (%d).", maxVertices);
		return false;
	}
	mesh.polygons = (unsigned short*)rcAllocate(sizeof(unsigned short)*maxTris*nvp*2, RC_ALLOC_PERMANENT);
	if (!mesh.polygons)
	{
		ctx->log(RC_LOG_ERROR, "rcBuildPolyMesh: Out of memory 'mesh.polys' (%d).", maxTris*nvp*2);
		return false;
	}
	mesh.regionIds = (unsigned short*)rcAllocate(sizeof(unsigned short)*maxTris, RC_ALLOC_PERMANENT);
	if (!mesh.regionIds)
	{
		ctx->log(RC_LOG_ERROR, "rcBuildPolyMesh: Out of memory 'mesh.regs' (%d).", maxTris);
		return false;
	}
	mesh.areaIds = (unsigned char*)rcAllocate(sizeof(unsigned char)*maxTris, RC_ALLOC_PERMANENT);
	if (!mesh.areaIds)
	{
	...
	rcScopedDelete<int> nextVert((int*)rcAllocate(sizeof(int)*maxVertices, RC_ALLOC_TEMPORARY));
	if (!nextVert)
	{
	...
	rcScopedDelete<int> firstVert((int*)rcAllocate(sizeof(int)*VERTEX_BUCKET_COUNT, RC_ALLOC_TEMPORARY));
	if (!firstVert)
	{
	...
	rcScopedDelete<int> indices((int*)rcAllocate(sizeof(int)*maxVertsPerCont, RC_ALLOC_TEMPORARY));
	if (!indices)
	{
		ctx->log(RC_LOG_ERROR, "rcBuildPolyMesh: Out of memory 'indices' (%d).", maxVertsPerCont);
		return false;
	}
	rcScopedDelete<int> tris((int*)rcAllocate(sizeof(int)*maxVertsPerCont*3, RC_ALLOC_TEMPORARY));
	if (!tris)
	{
		ctx->log(RC_LOG_ERROR, "rcBuildPolyMesh: Out of memory 'tris' (%d).", maxVertsPerCont*3);
		return false;
	}
	rcScopedDelete<unsigned short> polys((unsigned short*)rcAllocate(sizeof(unsigned short)*(maxVertsPerCont+1)*nvp, RC_ALLOC_TEMPORARY));
	if (!polys)
	{
...
/*			printf("\tconst float boundMin[3] = {%ff,%ff,%ff};\n", cset.boundMin[0], cset.boundMin[1], cset.boundMin[2]);
	...
	mesh.flags = (unsigned short*)rcAllocate(sizeof(unsigned short)*mesh.polygonsCount, RC_ALLOC_PERMANENT);
	if (!mesh.flags)
	{
...
/// @see rcAllocatePolyMesh, rcPolyMesh
bool rcMergePolyMeshes(rcContext* ctx, rcPolyMesh** meshes, const int nmeshes, rcPolyMesh& mesh)
{
	...
	mesh.verticesCount = 0;
	mesh.vertices = (unsigned short*)rcAllocate(sizeof(unsigned short)*maxVerts*3, RC_ALLOC_PERMANENT);
	if (!mesh.vertices)
	{
	...
	mesh.polygonsCount = 0;
	mesh.polygons = (unsigned short*)rcAllocate(sizeof(unsigned short)*maxPolys*2*mesh.maxVerticesPerPolygon, RC_ALLOC_PERMANENT);
	if (!mesh.polygons)
	{
	...
	mesh.regionIds = (unsigned short*)rcAllocate(sizeof(unsigned short)*maxPolys, RC_ALLOC_PERMANENT);
	if (!mesh.regionIds)
	{
	...
	mesh.areaIds = (unsigned char*)rcAllocate(sizeof(unsigned char)*maxPolys, RC_ALLOC_PERMANENT);
	if (!mesh.areaIds)
	{
	...
	mesh.flags = (unsigned short*)rcAllocate(sizeof(unsigned short)*maxPolys, RC_ALLOC_PERMANENT);
	if (!mesh.flags)
	{
	...
	rcScopedDelete<int> nextVert((int*)rcAllocate(sizeof(int)*maxVerts, RC_ALLOC_TEMPORARY));
	if (!nextVert)
	{
	...
	rcScopedDelete<unsigned short> vremap((unsigned short*)rcAllocate(sizeof(unsigned short)*maxVertsPerMesh, RC_ALLOC_PERMANENT));
	if (!vremap)
	{
...
bool rcCopyPolyMesh(rcContext* ctx, const rcPolyMesh& src, rcPolyMesh& dst)
{
	...
	dst.vertices = (unsigned short*)rcAllocate(sizeof(unsigned short)*src.verticesCount*3, RC_ALLOC_PERMANENT);
	if (!dst.vertices)
	{
	...
	dst.polygons = (unsigned short*)rcAllocate(sizeof(unsigned short)*src.polygonsCount*2*src.maxVerticesPerPolygon, RC_ALLOC_PERMANENT);
	if (!dst.polygons)
	{
	...
	dst.regionIds = (unsigned short*)rcAllocate(sizeof(unsigned short)*src.polygonsCount, RC_ALLOC_PERMANENT);
	if (!dst.regionIds)
	{
	...
	dst.areaIds = (unsigned char*)rcAllocate(sizeof(unsigned char)*src.polygonsCount, RC_ALLOC_PERMANENT);
	if (!dst.areaIds)
	{
	...
	dst.flags = (unsigned short*)rcAllocate(sizeof(unsigned short)*src.polygonsCount, RC_ALLOC_PERMANENT);
	if (!dst.flags)
	{
